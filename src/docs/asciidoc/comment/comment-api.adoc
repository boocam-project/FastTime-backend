...
=== Guide
- 마이페이지에서 회원이 남긴 댓글 목록 조회는 memberId를 요청 파라미터에 담아 요청해주십시오.
- 게시글 상세 페이지에서 게시글의 댓글 목록 조회는 articleId를 요청 파라미터에 담아 요청해주십시오.
- 댓글에 대한 대댓글 조회는 parentCommentId를 파라미터로 담아 요청해주십시오.
